...
<<<<<<< HEAD:Screens/Tasks/Maze/PreMaze.py
# Haptic on_off
...
=======
>>>>>>> parent of 7136d27 (Input boxes response to enter):PreMaze.py
...
def create_draw_area(draw_width, draw_height):
    ...
    draw_area = DrawArea.DrawArea(
        draw_width, draw_height, start_pos, (0, 0, 0))
    return draw_area
...
try:
    while True:
        ...
        if current != -1:
            mazes[current].draw_maze(
                screen, (255, 255, 255), start_pos[0], start_pos[1], skip, show)
...
<<<<<<< HEAD:Screens/Tasks/Maze/PreMaze.py
                            current].saved:
                        mazes[current].save_maze(
                            screen, start_pos[0], start_pos[1])
...
>>>>>>> parent of 7136d27 (Input boxes response to enter):PreMaze.py
                        ...
                        for line in mazes[current].grid[mazes[current].index(i, j)].lines:
                            if line.collidepoint(pygame.mouse.get_pos()):
                                ...
                                if len(draw_areas[current].points) - 1 != -1:
                                    draw_areas[current].breaks.append(
                                        len(draw_areas[current].points) - 1)
                draw_areas[current].interact(
            ...
            if e.type == pygame.MOUSEBUTTONDOWN and buttons[1].rect.collidepoint(e.pos):
                ...
                if buttons[1].txt == 'New Maze':
                    ...
                    draw_areas.append(create_draw_area(
                        mazes[current].draw_width, maze.draw_height))
