...
prev_menu_btn = Button.Button('img', ['go-back-arrow.png'], (50, 50),
                              'Prev', False, 50, [(0, 0, 0)], (screen_width / 15, screen_height / 9))
...
for i in range(len(buttons)):
    buttons[i].move((xs + padding + ((xl - 2 * padding) /
                    (len(buttons) - 1)) * i, draw_area.canvas.bottom + padding))
...
def draw_solution(current):
    ...
    for i in range(len(lettersX[current])):
        pygame.draw.circle(screen, (100, 100, 100),
                           (lettersX[current][i], lettersY[current][i]), 10)
    ...
    while True:
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.MOUSEBUTTONDOWN and prev_menu_btn.rect.collidepoint(e.pos):
                raise StopIteration
            draw_area.interact(pygame.mouse.get_pos(),
